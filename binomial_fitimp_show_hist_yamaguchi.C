...
/////////Main function///////////////////////////////////
void show_hist(const char* filename){
  ...
  while(ch <= ch_max){
	...
	c1->Divide(4,4);
      ...
      hist->SetTitle(name);
      hist->GetYaxis()->SetRangeUser(0,260);
      hist->Draw();
  ...
  for( int ch = 0;ch<=ch_max;ch++){
	...
	//give the binomial error : editted by S.Ochiai 2023.02.16
	Double_t error[300];
	int nbins = hist_scurve->GetXaxis()->GetNbins();
	for(int i=1;i<nbins;i++){
		double y = hist_scurve->GetBinContent(i);
		error[i-1] = binomial_err(y); 
	}
	hist_scurve->SetError(error); //Set Binomial Errorto BinErorr
	...
	for ( Int_t i = 2; i < 11; i++ ){
		...
		//----------------------------
		//out << d << endl;
		//string writing_text1 = Form("Chi2 : %lf", erf->GetChisquare());
		//string writing_text2 = Form("NDF : %d", erf->GetNDF());
		//out << writing_text1 << endl;
		//out << writing_text2 << endl;
		//out << endl;
		//----------------------------
