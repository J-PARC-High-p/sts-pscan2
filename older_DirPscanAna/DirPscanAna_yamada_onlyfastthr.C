...
void DirPscanAna(const char* dirpath){
    ...
    while( dp != NULL ){//--1111111-- Dir Scan Loop Layer1 --1111111--
        ...
        if( RootFilePath == ".root"){//--222222-- if(root file) Layer2 --2222222--
            ...
            while(ch <= ch_max){//--3333333-- Layer3-1 --3333333--
                for( int itmp = 0; itmp < 16;itmp ++){
                    ...
                    if ( func != NULL ){
                        ...
                        double fast_noise_dcnt = ENC(func->GetParameter(2));
            ...
            c1->Clear();
            c1->cd(1);
            int pageCount = 0; // 現在のページに描画されたヒストグラムの数を追跡

            for(int ch = 0; ch <= ch_max; ch++){
                // ページの最初のチャネルまたは新しいページの開始時
                if(pageCount == 0){
                    c1->Clear();
                    c1->Divide(2, 4);
                }
                ...
                // Canvas上の正しい位置を計算
                int padNumber = pageCount + 1;
                c1->cd(padNumber);

                // d_maxの時のヒストグラムを描画
                TString name_scurve = TString::Format("h_scurve_%d_%d", ch, d_max);
                TH1D* hist_scurve_2 = (TH1D*)file->Get(name_scurve);
                hist_scurve_2->SetTitle(name_scurve);

                        //give the binomial error : eddited by S.Ochiai 2023.03.01
                        Double_t error[300];
                        int nbins = hist_scurve_2->GetXaxis()->GetNbins();
                        for(int i=1; i<nbins; i++){
                            double y = hist_scurve_2->GetBinContent(i);
                            error[i-1] = binomial_error(y);
                        }
                        hist_scurve_2->SetError(error); //Set Binomial Error to BinError

                        TF1* erf = new TF1("erf", "200*ROOT::Math::gaussian_cdf(x,[0],[1])",0,300);
                        erf->SetParameters(20,160);
                        hist_scurve_2->Rebin(4);
                        hist_scurve_2->Draw("HIST");
                        hist_scurve_2->Fit("erf","LQ","",0,255);//Fitting #1
                ...
                //----------------------added by S.Ochiai 2023.02.03------------------
                        ...
                        for ( Int_t i = 2; i < 11; i++ ){
                            ...
                            erf->SetParameters(sigma, x0);
                            hist_scurve_2->Fit("erf", "LQ", "", xlow, 255);
                            chi2_current = erf->GetChisquare();
                            if( chi2_current > chi2_previous || TMath::Abs( chi2_previous - chi2_current ) < chi2_previous * 0.01){
                                break;
                            }
                        ...
                        double chi2 = chi2_previous;
                        int NDF = erf->GetNDF();//自由度
                        int bin1 = hist_scurve_2->GetBinContent(1);
                        //sig_ch[ch]->Fill(d,sigma);
                            ...
                            TString name_dcnt = TString::Format("h_d_%d_31",ch);
                            TH1D* hist_fast = (TH1D*)file->Get(name_dcnt);
                            hist_fast->Rebin(4);
                            hist_fast->Fit("gaus","LQR0","",25,100);
                            TF1* func = hist_fast->GetFunction("gaus");
                            int fast_trim = 36;
                ...
                // ページに描画されたヒストグラムの数を更新
                pageCount++;

                // ページがいっぱいになったら、PDFに追加してカウンターをリセット
                if(pageCount == 8 || ch == ch_max){
                    c1->Draw();
                    c1->Update();
                    c1->Print(out_pdf, "PDF");
                    pageCount = 0; // ページカウントをリセット
                }
            }
